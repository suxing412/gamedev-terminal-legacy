...
// 事件的「种」：把里面会变的数字抹平，剩下的就是它在讲哪件事。
// `值守心跳 seq=1456 应有=7 周期=5m` 与 `seq=1457` 是同一件事，不是两件。
const 事种 = (文) => String(文 || '')
  .replace(/\d[\d.,:]*/g, '#')      // 所有数字归一
  .replace(/\s+/g, ' ')
  .trim();
...
function 折叠(事们) {
  ...
  for (const e of (事们 || [])) {
    ...
    if (尾 && 尾.种 === 种) {
      尾.次 += 1;
      尾.起时 = e.时;                 // 列表是新在前，所以后来的是更早的
      continue;
    }
    出.push({ 种, 时: e.时, 起时: e.时, 文: e.文, 次: 1 });
  }
  return 出;
}
