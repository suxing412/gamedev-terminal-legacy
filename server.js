...
// 进程起于什么时候。换装时用得着：**版本号相同但起于是旧时刻，说明杀旧那步没干净**。
const 启动于 = Date.now();

// ---- 版本口 ----
//
// **换装仪式第 7 条要求拿它确认，而终端一直没有这个口。**
// 那条仪式是 2026-08-28 一次假换装之后立的：那次打完包、起完进程、探到
// /api/attn 有正常响应，就报告「0.40.1 活体已起」——而应答的是没被杀掉的 0.40.0。
// 旧实例和新实例长得一模一样，**只有版本号分得开**。
// 仪式于是写死：「禁止拿『API 有响应』当数，必须比版本号」。
//
// 可这个口只有监制台有，终端没有——2026-09-01 实测 `/api/version` 回 404。
// 也就是说这条仪式在终端上**从来执行不了**，而每次换装的记录都写着「已确认」。
// 一条做不到的规矩，比没有这条规矩坏：它让人以为验过了。
//
// 版本从 package.json 读，不写死——写死的那个数迟早和真的分家。
app.get('/api/version', (req, res) => {
  let 版本 = null;
  try { 版本 = require('./package.json').version || null; } catch (e) { 版本 = null; }
  res.json({
    版本,
    起于: new Date(启动于).toISOString(),
    // 打包态与源码态要分得开：换装时看的是 exe，开发时看的是源码跑的那份
    形态: process.env.PORTABLE_EXECUTABLE_DIR ? 'exe' : '源码',
  });
});
...
app.get('/api/events', (req, res) => {
  ...
  try {
    ...
    //
    // **日期要带上。**窗口是 600 行 ≈ 58 小时，可这里原本只切 HH:MM。
    // 实测那一屏折成 51 组，其中 30 组不是当天：
    // 「03:49 值守塔阵亡：连续 290 拍无在位回执」真实时刻是两天前的 03:49，
    // 上下两行标着 05:11 与 03:46——**时刻局部递减，而屏上没有任何提示**。
    // 跨午夜的折叠行更直白：`×39 23:03→02:13`，起点看着比终点还晚。
...
// 席况台：每一席此刻在做什么（2026-09-02 批六）。
//
// 进程内一张表，**不落盘**——它描述的是「此刻」，而此刻这件事没有比进程更长的寿命：
// 终端重启了，「正在答话」就该消失。落盘反而会把一个死掉的状态留到下一条命里，
// 而屏上一个永远在答话的幽灵，比这一栏整个不存在还坏。
//
// 声明放在 /api/say 之前是**为了读得顺**：运行时它怎么摆都不会炸
// （handler 是请求时才跑，那时模块早求值完了），但源码里「先用后声明」
// 读起来是错的，而下一个人照着这个位置往上加东西时就未必还安全。
const 席况lib = require('./server/lib/席况');
const 席况台 = 席况lib.开台();
...
app.post('/api/say', async (req, res) => {
  ...
  // 席况（2026-09-02 批六）：在座栏那行「在做什么」的来源就在这条流上。
  // 开始 / 在做 / 结束 三处必须配对——**漏掉 结束 就会在栏里留下一个永远在答话的幽灵**，
  // 而那正是这块屏最不能有的东西：一个看着在动、其实早就没人的状态。
  ...
  let 断 = false;
  const 席键 = 席 && 席.名;
  席况台.开始(席键, Date.now(), 来路);
  // 客户端断线：下一拍就收摊，别继续烧额度。**席况也要当场清**——
  // 等到 finally 才清的话，断线到收摊之间那一段屏上还写着「正在答话」。
  res.on('close', () => { 断 = true; 席况台.结束(席键); });
  ...
  try {
    ...
    // **三条出路都要清**：跑完、断线、抛异常。漏一条就留一个永远在答话的幽灵，
    // 而幽灵不会自己消失——它得等到下一次这一席真的说话才被覆盖。
    席况台.结束(席键);
  }
...
// 席位目录（2026-09-02 批六）：自定义席位一席一份协议文档，落在这里。
//
// **锚在 terminal 文稿根，不锚 终端根。** 终端根在源码态是仓根、在 portable exe 态是部署区，
// 两处各写各的，于是「源码态建的席位，换装之后不见了」——
// 与今天上午刚修的 排除目录 那条同型。文稿根表里的 terminal 路在两种形态下是同一个，
// 所以席位在两边也是同一批，而且它天然落在文稿台的可编辑范围内
// （制作人的话：「可以直接在自定义中跳转到文档库中开编辑器进行编辑」）。
function 席位目录() {
  const t = 文稿根表().find((r) => r && r.键 === 'terminal');
  return t && t.路 ? path.join(t.路, '席位') : path.join(终端根, '席位');
}
require('./server/lib/坐席').挂目录(席位目录());
// 席位页：一席一份协议文档，页面只做「看 + 去文稿台改」，故意没有表单。
require('./server/routes/席位页').挂(app, { 目录: 席位目录 });
...
function 文稿列举() {
  ...
  // **不能只排当前这一个**：终端根在 exe 态是部署区、在源码态是仓根，
  // 两边各留过一份工作目录，排一个漏一个（0.9.0 换装当场破功，详见 工作目录们 的注释）。
  const 根 = 文稿根表();
  const 表 = 文稿lib.列举(根, { 排除目录: 文稿lib.工作目录们(终端根, 根) });
...
app.get('/api/seats', (req, res) => {
  const 坐席 = require('./server/lib/坐席');
  const 现在 = Date.now();
  // 席况（2026-09-02 批六）：**只报终端自己确知的两件事**——
  // 此刻有没有一条 /api/say 在为这一席跑（跑了多久、坐席自己报的"在做什么"），
  // 以及上一次说话是什么时候（会话档 mtime）。
  // 别的一律不报：在座栏上一条编出来的状态，比那一栏整个不存在还坏。
  res.json({
    // **名单() 不是 全部**：全部 是内建那七席，名单 含自定义席位（协议档扫来的）。
    // 两个名字各有各的意思，混用就会出现「新建的席位在这一栏看得见、在那一栏看不见」。
    席: 坐席.名单().map((x) => {
      const 况 = 席况台.况(x.名, 现在);
      const 末 = 席况lib.末话(终端根, x.名);
      return {
        名: x.名, 接模型: !!x.接模型, 人设: x.人设 || '',
        自建: !!x.自建, 模型: x.模型 || null, 档: x.档 || null,
        在答: !!况, 做: (况 && 况.做) || null, 答了秒: 况 ? 况.秒 : null,
        // 读不到就是 null，**不是 0**：「从来没说过话」和「刚说完」不能长成一个样子
        末话: 末 == null ? null : new Date(末).toISOString(),
      };
    }),
    // 群里那条线（没点名某一席）也可能在飞，它不属于任何一席
    群: 席况台.况(null, 现在),
    于: new Date(现在).toISOString(),
  });
});
