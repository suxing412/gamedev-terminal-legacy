 ...
 */
/**
 * 静默多久(末条, 现在, 阈值毫秒) → { 毫秒, 说 } 或 null
 *
 * **「末条写了一半」与「末条写完了但已经三天没人说话」是两件事。**
 * 原来只有前者（末行陈旧，看 JSON 闭没闭合），于是线程静默三天时这一页
 * 看起来一切正常：有内容、有计数、没有警告。
 * 而「这条线还活着没有」正是打开这一页要问的第一件事。
 *
 * 阈值默认 12 小时：这条线本来就不是每小时都有人说话的，报得太勤等于噪声。
 */
function 静默多久(末条, 现在, 阈值毫秒 = 12 * 3600 * 1000) {
  const t = 末条 && 末条.t ? Date.parse(末条.t) : NaN;
  if (!Number.isFinite(t)) return null;
  const 龄 = 现在 - t;
  if (!(龄 >= 阈值毫秒)) return null;
  const 时 = 龄 / 3600000;
  const 说 = 时 >= 48 ? `${Math.floor(时 / 24)} 天` : `${Math.round(时)} 小时`;
  return { 毫秒: 龄, 说 };
}
...
module.exports = { 读, 算ID, 是播报, 是产出物, 逻辑行数, 形状不合, 末行陈旧, 静默多久, 播报前缀, 产出词 };
