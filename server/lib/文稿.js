...
const 类别表 = [
  {
    键: 'zaiban', 名: '在办文稿', 释: '你我来回改的：方案、评审、验收、设计文档',
    配: [/(^|\/)(方案|评审|外审|验收|需求定案|设计文档|收口报告|拆单简报|想法)/, /设计文档\.md$/i],
  },
  {
...
function 归类(根键, 相对) {
  ...
  for (const c of 类别表) if (c.配.some((re) => re.test(s))) return c.键;
  return 'qita';
}
...
function 记号统计(列表, 根路, 缓 = new Map()) {
  for (const it of (列表 || [])) {
    if (!it.可写) { it.记 = null; continue; }        // 只读的不扫：他不会去标
    const 根 = 根路[it.根];
