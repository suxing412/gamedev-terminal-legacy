...
// 只借它的 frontmatter 解析。渲染器与这里读的是同一份文件头，**同一把尺**——
// 两处各写一个解析器，就一定会有一天对同一个文件头给出两种答案。
 ...
 * 认用途(值) → 类别键 或 null
 *
 * 手工覆写用的。制作人在文档头的 frontmatter 里写一行 `用途: 规章`，
 * 就压过下面那套按文件名猜的规则。**猜错了要有救**——这是 2026-09-01 他拍板加的：
 * 规则再准也有猜不中的（实测 68/73），而猜不中的那几份此前没有任何办法纠正。
 *
 * 名与键都收（他会写「规章」，不会写「guizhang」），大小写与前后空白都洗掉。
 * **认不出就返回 null，回落到猜**——不许因为写错一个字就把文档丢进「其它」，
 * 那比不给覆写更坏：他改了一行，文档反而找不着了。
 */
function 认用途(值) {
  const v = String(值 == null ? '' : 值).trim().toLowerCase();
  if (!v) return null;
  for (const c of 类别表) {
    if (c.键.toLowerCase() === v) return c.键;
    if (String(c.名).toLowerCase() === v) return c.键;
  }
  if (v === '其它' || v === '其他' || v === 'qita') return 'qita';
  return null;
}
 ...
 */
/**
 * 文稿台自己的工作目录（草稿 / 版本环 / 锁）落在 `<终端根>/文稿`。
 * 它必须被排除，否则文件库会**把自己的草稿和版本历史当成文档列出来**。
 *
 * **但「终端根」在源码态和 portable exe 态是两个不同的目录**，
 * 而排除表只填了当前这一个——于是 0.9.0 换装后当场破功：
 * exe 的 终端根 是部署区 `D:/GitHub/AI-GameStudio/终端`，
 * 排的是 `<部署区>/文稿`（一个还不存在的目录，等于什么都没排），
 * 而源码态跑出来的那份工作目录还留在仓里 `D:/GitHub/gamedev-terminal/文稿`——
 * 它在 terminal 根的**里面**，于是三份草稿被当成文档列进了文件库，
 * 还被记号栏数成「3 份有记号」。制作人看到的那个数字，三份全是假的。
 *
 * 这是 portable-exe 路径陷阱的又一次复发，而且是**同一段注释里已经写明危险、
 * 却选了个照样落进去的取法**——上面那条 排 的注释把机理讲得很清楚。
 * 所以这里不再猜「工作目录在哪」，而是把**所有可能宿主**一起排：
 * 当前终端根一个，terminal 根一个（源码态的历史落点）。
 * 仓里的 `文稿/` 本来就在 .gitignore 里——它任何时候都不是正经文档目录。
 */
function 工作目录们(终端根, 根表) {
  const 出 = [];
  const 加 = (p) => { if (p) 出.push(path.join(p, '文稿')); };
  加(终端根);
  for (const r of (Array.isArray(根表) ? 根表 : [])) {
    if (r && r.键 === 'terminal' && r.路) 加(r.路);
  }
  // 去重按归一后的口径（Windows 大小写不敏感 + 斜杠方向）
  const 见 = new Set();
  return 出.filter((p) => { const g = 归一(p); if (见.has(g)) return false; 见.add(g); return true; });
}
...
function 记号统计(列表, 根路, 缓 = new Map()) {
  ...
  for (const it of (列表 || [])) {
    ...
    本轮.add(全);
    const 旧 = 缓.get(全);
    if (旧 && 旧.改于 === it.改于) { it.记 = 旧.记; it.头 = 旧.头 || null; continue; }
    let 文;
    try { 文 = fs.readFileSync(全, 'utf8'); } catch (e) { it.记 = null; it.头 = null; continue; }
    const 计 = 记号计(扫记号(文));
    // **顺手把 frontmatter 也读了。**这一趟已经把全文读进内存了，
    // 再开一趟只为拿文件头是白读一遍盘（实测全库一趟 48ms，两趟就是 96ms）。
    const 头 = 拆头(文).头;
    it.记 = 计;
    it.头 = 头;
    缓.set(全, { 改于: it.改于, 记: 计, 头 });
  }
  // ── 用途覆写 ──
  //
  // 归类() 是纯函数、只吃路径，它读不到文件内容——所以覆写只能落在这里，
  // 也就是**唯一一处已经把全文读进来的地方**。
  //
  // 覆写发生在猜之后：猜的结果先写进 it.类（列举 里做的），这里再按 用途 压过它。
  // 覆写掉的话把原来猜的留在 it.猜类 里——**屏上要说得出「这是你定的，不是我猜的」**，
  // 不然他下次看到分类不对，分不清是规则错了还是他自己写错了。
  for (const it of (列表 || [])) {
    const 定 = it.头 && 认用途(it.头.用途);
    if (定 && 定 !== it.类) { it.猜类 = it.类; it.类 = 定; it.定类 = true; }
    else if (定) { it.定类 = true; }
  }

  // 淘汰：**本轮没见到的删掉**，不设条数上限。
  //
  // 原来是「超过 2000 就只留最后 1200 条」。那是 FIFO 不是 LRU——
  // 命中缓存时不会重新 set，插入序就是列举序，于是文件数一过 2000：
  // 每轮淘汰掉固定的那 800 条 → 下一轮把它们重读一遍重新插到末尾 → 淘汰掉另外 800 条。
  // **每次列举从「读 0 个文件」跳成「读 800 个」，而且永远回不去。**
  // 断崖之外还有一层：那 800 条到底是哪 800 条，取决于列举序，谁也说不清。
  //
  // 而这里根本不需要猜：`列举()` 每次都把活文件的全集交到手上了。
  // 不在这份全集里的键就是已经删掉/改名/移出根的文件——直接删，O(n)，没有断崖。
  for (const k of [...缓.keys()]) if (!本轮.has(k)) 缓.delete(k);
  return 缓;
}
...
module.exports = {
  认用途,
  默认根表, 默认剪枝, 记号们,
  取根, 校路径, 可写, 列举, 扫记号, 记号计, 搜, 读, 在根内, 软链判, 工作目录们,
