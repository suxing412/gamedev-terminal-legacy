 ...
 *
 * ── 闸的由来（曾经只有约定）──────────────────────────────────
 * 首版这段文案写着「硬拦在 server 侧，写了也会被拒」，而 2026-08-31 验收复核
 * grep 出来：外部可写() 与 请求解锁() 在生产代码里**零调用点**——
 * 只有定义、两句声称它是硬闸的注释、和两个直接调它的判据。
 * **判据全绿，因为判据自己直接调那个函数。**这正是 H104 要防的那种假判据。
 *
 * 当晚制作人裁「外部可写要加」，于是接上了：server.js 的 坐席选项() 里
 * 挂了一个 PreToolUse hook（走 SDK 的 options.hooks，不动全局 settings.json），
 * 判定在 lib/写手闸.js。所以现在这句话说的是实情，不是承诺。
 *
 * 告示与闸**两个都要**：能靠机器挡的不要只靠自觉，
 * 但也别让坐席被挡完之后不知道自己为什么被挡、下一步该干什么。
 */
function 告示(表, 现在 = Date.now()) {
  ...
  return '【文稿台占用中】以下文件此刻正被人在终端里编辑，**你可以读，但不要写**；'
    ...
    + '所以别绕道去写——绕过去也只是把他正在改的东西盖掉，而他未必立刻看得出来。）\n\n';
}
