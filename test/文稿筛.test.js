...
// ── 版本历史：看得到，还要拿得回来（评审 S15）──────────────────
//
// 案发：冲突框承诺「盘上那版已经存进版本历史，可以找回」，而历史面板只是把全文
// 塞进一个 26vh 高的滚动 div——**没有恢复、没有对比、没有复制**。
// 承诺了一条路，路口却没有门。而 `window.文稿编辑.画差异` 就在手边
// （冲突框正在用它），`/api/doc/version` 也已经把全文回给前端了。

test('史① 三条路都在：全文 / 和当前比 / 用这一版覆盖编辑器', () => {
  const 源 = fs.readFileSync(path.join(__dirname, '..', 'public', '文稿.js'), 'utf8');
  const i = 源.indexOf('function 画史文');
  assert.ok(i > 0, '没有 画史文 —— 历史面板还是那个只会塞全文的 div');
  // **光有这个函数不算**：它得真的被版本详情那一支调用。
  // 第一版只验了「函数在」，于是把调用点换回 `文格.textContent = j.文` 之后判据还是绿的。
  assert.match(源, /史文 = \{ 档: b\.getAttribute/, '版本详情没把选中那一版记下来');
  assert.match(源, /画史文\('全'\)/, '版本详情没走 画史文 —— 三条路一条都到不了');
  const 段 = 源.slice(i, i + 1400);
  assert.match(段, /data-模="全"/, '缺「全文」');
  assert.match(段, /data-模="比"/, '缺「和当前比」');
  assert.match(段, /class="史用"/, '缺「用这一版覆盖编辑器」');
  assert.match(段, /画差异/, '对比没有复用冲突框那个 画差异');
});

test('史② **不在编辑态时，「和当前比」与「覆盖」要禁掉并说明原因**', () => {
  // 不在编辑时页面上是渲染好的 HTML、不是 markdown，拿它去比会得出一堆假差异——
  // **那比不给对比更坏**。禁掉可以，禁了不说为什么不行。
  const 源 = fs.readFileSync(path.join(__dirname, '..', 'public', '文稿.js'), 'utf8');
  const i = 源.indexOf('function 画史文');
  const 段 = 源.slice(i, i + 1400);
  assert.match(段, /可比 \?\s*''\s*:\s*' disabled title=/, '「和当前比」禁掉时没给 title 说原因');
  assert.match(段, /编 && 编\.编器 \?\s*''\s*:\s*' disabled title=/, '「覆盖」禁掉时没给 title 说原因');
  assert.match(段, /源码/, '没说清「不在编辑时拿到的是 HTML 不是源码」这个原因');
});

test('史③ **覆盖只灌进编辑器，不落盘**（落盘仍走存盘闸）', () => {
  const 源 = fs.readFileSync(path.join(__dirname, '..', 'public', '文稿.js'), 'utf8');
  const i = 源.indexOf("const 用 = e.target.closest('.史用')");
  assert.ok(i > 0, '找不到覆盖分支');
  // **只取这一支本身**：取宽了会读进下一个分支（那一支本来就要 fetch 取版本全文），
  // 判据就会为了别人的代码而红。第一版取了 900 字符，正好越界。
  const j = 源.indexOf("const b = e.target.closest('.史条')", i);
  assert.ok(j > i, '找不到这一支的结尾');
  const 段 = 源.slice(i, j);
  assert.match(段, /编\.编器\.设文\(史文\.文\)/, '没把那一版灌进编辑器');
  assert.match(段, /编\.脏 = true/, '灌完没标脏 —— 那样存盘钮不会亮，改动会被当成没发生');
  assert.ok(!/fetch\(|存盘\(/.test(段), '覆盖那一步直接落盘了 —— 它该只摆到面前，不替他决定');
  assert.match(段, /await 问\(/, '覆盖前没确认 —— 它会顶掉当前未保存的改动');
});

test('史④ 冲突框那句承诺要说得出「怎么走」', () => {
  // 只说「可以找回」而不说从哪找，等于把问题丢给他自己猜 —— 那正是本仓提案里「缺出口」那一型。
  const 源 = fs.readFileSync(path.join(__dirname, '..', 'public', '文稿.js'), 'utf8');
  // **按它的语法上下文定位，不按字面量 indexOf。**
  // 那句话在这个文件里出现三次：两次在注释里（讲它曾经是句空话），一次是真的要显示的字符串。
  // indexOf 找到的是第一次——注释——而注释里同样有「历史」和「覆盖」两个字，
  // 于是判据永远绿。这是第一版犯的错。
  const m = 源.match(/\?\s*'(盘上那版已经存进版本历史[^']*)'/);
  assert.ok(m, '那句承诺不见了（或者不再是一个字符串字面量）');
  const 句 = m[1];
  assert.match(句, /历史/, '没说从哪找');
  assert.match(句, /覆盖/, '没说找到之后怎么用回来');
});
