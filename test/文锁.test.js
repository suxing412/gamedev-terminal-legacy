...
test('守⑪b 有人锁着时告示说得出是哪份、谁锁的；没人锁着时是空串', () => {
  ...
  // 2026-08-31 晚：制作人裁「外部可写要加」，闸已接上（坐席选项() 的 PreToolUse），
  // 所以告示重新可以说「会被拒」——但**必须说清拦在哪一层**，
  // 不能再退回首版那句没有出处的「硬拦在 server 侧」。
  // 判据的方向也跟着翻了一次：先前它在防「声称有闸而闸不存在」，
  // 现在防的是「闸接上了却不告诉坐席，于是它被挡了不知道为什么」。
  assert.ok(/机器闸|PreToolUse/.test(g), '告示没说清这条有机器闸：' + g);
  assert.ok(/解锁请求|人闸/.test(g), '告示没告诉坐席「被挡之后会发生什么」：' + g);
  // 过期的锁不该还在告示里
