...
test('守② 未接模型的席位不许 disabled（disabled 的按钮点了完全没反应）', () => {
  ...
  // 未接的席位要有 title 说清为什么开不了私聊。
  // **不钉表达式的形状，钉那句话在不在**——2026-09-02 批六把 title 抽成了 提示 变量
  // （因为在答态那一行也要用它），判据当场红了而性质一个字没变。
  // 一条钉着写法的判据，会在每一次正当重构时假报一次，而假报两次之后就没人信它了。
  assert.match(段, /未接[\s\S]{0,120}还没接模型[\s\S]{0,80}私聊/,
    '未接的席位没有一句说清"为什么开不了私聊"的话');
  assert.match(段, /title="\$\{esc\(提示\)\}"|未接 \?[^:]*title=/,
    '那句话没落到 title 上，等于只写在源码里给自己看');
});
