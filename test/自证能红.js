...
const 变异 = [
  ...
  // 【撤销记录 · 2026-09-02】上面这一族原有两条变异：
  //   · 「进塔前不松开 minWidth（半屏塔从来没窄过，形态钮落在屏外点不着）」
  //   · 「回全屏不装回 minWidth（三栏能被拖成 360 宽）」
  // 两条锚在 main.js 的 ipcMain('形态:半屏') 那段上。制作人 09-02 拍板删除半屏，
  // 那段代码不复存在，锚点失效——**不是判据失灵，是被守的实现没了**。
  // 连同它们守的 壳接线.test.js 接① / 接①b 一并撤销。
  //
  // 记在这里而不是悄悄删：变异总数从 209 掉到 207，看数字会以为漏了两条。
  // 半屏那三处（按钮 / F9 / 壳桥）改由 跑道.js 的 S19①–④ 在真壳里盯着，
  // 那四条验的是行为（按 F9 之后类变没变、窗宽被不被钳走），不是源码文本。
