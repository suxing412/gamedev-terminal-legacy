...
test('守⑥c 数字键选的是**看得见的**第 N 条，不是闸表的第 N 项', () => {
  // 折叠之后这两者会分家：折起来的还在闸表里。按 2 带上一条屏幕上没有的单，
  // 是"点了有反应、但反应的不是你指的那个"——比没反应更难查。
  const src = fs.readFileSync(path.join(__dirname, '..', 'public', 'app.js'), 'utf8');
  const i = src.indexOf("e.key >= '1' && e.key <= '9'");
  assert.ok(i > 0, '找不到数字键分支');
  const 段 = src.slice(i, i + 400);
  assert.match(段, /offsetParent/, '要按可见性过滤（offsetParent === null 即被折叠或隐藏）');
  assert.ok(!/上带\(Number\(e\.key\) - 1\)/.test(段), '还在直接拿按键数当闸表下标');
});
